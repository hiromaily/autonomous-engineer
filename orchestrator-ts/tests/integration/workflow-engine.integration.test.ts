...
import type { WorkflowState, WorkflowPhase } from '../../domain/workflow/types';
import type { WorkflowEvent } from '../../application/ports/workflow';
import type { PhaseRunner, PhaseResult } from '../../domain/workflow/phase-runner';
...
function makeStubPhaseRunner(opts?: {
  ...
  const runner: PhaseRunner = {
    ...
    onEnter: mock(async (phase: WorkflowPhase) => { enteredPhases.push(phase); }),
...
describe('WorkflowEngine integration — SPEC_INIT → REQUIREMENTS → paused_for_approval', () => {
  it('pauses at REQUIREMENTS when spec.json has requirements.approved=false', async () => {
    ...
    const engine = new WorkflowEngine({ stateStore, eventBus, phaseRunner, approvalGate: new ApprovalGate(), specDir, language: 'en' });
  ...
  it('emits phase:start and phase:complete for SPEC_INIT, then phase:start and phase:complete for REQUIREMENTS, then approval:required', async () => {
    ...
    const specInitStartIdx = capturedEvents.findIndex((e) => e.type === 'phase:start' && 'phase' in e && e.phase === 'SPEC_INIT');
    const reqStartIdx = capturedEvents.findIndex((e) => e.type === 'phase:start' && 'phase' in e && e.phase === 'REQUIREMENTS');
...
describe('WorkflowEngine integration — resume after REQUIREMENTS approval', () => {
  ...
  it('SPEC_INIT is NOT re-executed on resume after REQUIREMENTS approval', async () => {
    ...
    await writeSpecJson({ approvals: { requirements: { approved: true }, design: { approved: true }, tasks: { approved: true } }, ready_for_implementation: true });
    ...
    const resumeEngine = new WorkflowEngine({ stateStore, eventBus: new WorkflowEventBus(), phaseRunner: resumeRunner, approvalGate: new ApprovalGate(), specDir, language: 'en' });
  ...
  it('approval:required event is re-emitted when spec.json still has not approved on resume', async () => {
    ...
    const resumeRunner = makeStubPhaseRunner();
    const resumeEngine = new WorkflowEngine({ stateStore, eventBus: resumeBus, phaseRunner: resumeRunner, approvalGate: new ApprovalGate(), specDir, language: 'en' });
...
describe('WorkflowStateStore integration — persist and restore cycle', () => {
  it('restores each status variant correctly', async () => {
    const variants: WorkflowState[] = [
      { specName: SPEC_NAME, currentPhase: 'REQUIREMENTS', completedPhases: ['SPEC_INIT'], status: 'running', startedAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { specName: SPEC_NAME, currentPhase: 'REQUIREMENTS', completedPhases: ['SPEC_INIT'], status: 'paused_for_approval', startedAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { specName: SPEC_NAME, currentPhase: 'DESIGN', completedPhases: ['SPEC_INIT', 'REQUIREMENTS'], status: 'completed', startedAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { specName: SPEC_NAME, currentPhase: 'DESIGN', completedPhases: ['SPEC_INIT', 'REQUIREMENTS'], status: 'failed', failureDetail: { phase: 'DESIGN', error: 'SDD error' }, startedAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    ];
