...
import { mkdtemp, rm, mkdir, writeFile, access, readFile, readdir } from 'node:fs/promises';
...
describe('FileMemoryStore - Task 3.3: update()', () => {
  ...
  it('matches the entry title case-insensitively', async () => {
    ...
    const result = await store.update(target, 'MY RULE', makeEntry('MY RULE', {
  ...
  it('preserves other entries when updating one', async () => {
    ...
    await store.update(target, 'Entry B', makeEntry('Entry B', {
  ...
  it('does not leave a .tmp file after successful update', async () => {
    ...
    await store.update(target, 'Clean Entry', makeEntry('Clean Entry', {
      description: 'Updated.',
    }));
    await expect(
  ...
  it('handles update when file contains only one entry', async () => {
    ...
    const result = await store.update(target, 'Solo Entry', makeEntry('Solo Entry', {
...
describe('FileMemoryStore - Task 3.4: writeFailure() and getFailures()', () => {
  ...
  const makeRecord = (override?: Partial<import('../../../application/ports/memory').FailureRecord>): import('../../../application/ports/memory').FailureRecord => ({
    taskId: 'task-1',
...
describe('FileMemoryStore - Task 3.5: query() scoring and ranking', () => {
  ...
  it('default topN is 5 when not specified', async () => {
    const entries = Array.from({ length: 8 }, (_, i) =>
      buildEntry(`Entry ${i}`, `keyword appears ${i + 1} times: ${'keyword '.repeat(i + 1).trim()}`),
    ).join('\n---\n');
