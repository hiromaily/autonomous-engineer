...
describe('AgentLoopEvent discriminated union', () => {
  ...
  it('discriminates on the type field to allow exhaustive narrowing', () => {
    const events: AgentLoopEvent[] = [
      ...
      { type: 'iteration:complete', iteration: 1, category: 'Exploration', toolName: 'read_file', durationMs: 200, assessment: 'expected' },
    ];
...
const _exhaustiveLoopStep = (s: LoopStep): string => {
  switch (s) {
    ...
    case 'UPDATE_STATE': return 'UPDATE_STATE';
  }
};
...
const _exhaustiveTermination = (t: TerminationCondition): string => {
  switch (t) {
    ...
    case 'MAX_ITERATIONS_REACHED':     return 'MAX_ITERATIONS_REACHED';
    case 'HUMAN_INTERVENTION_REQUIRED': return 'HUMAN_INTERVENTION_REQUIRED';
...
const _exhaustiveAgentLoopEvent = (e: AgentLoopEvent): string => {
  switch (e.type) {
    case 'iteration:start':    return 'iteration:start';
    case 'iteration:complete': return 'iteration:complete';
    case 'step:start':         return 'step:start';
    case 'step:complete':      return 'step:complete';
    case 'recovery:attempt':   return 'recovery:attempt';
