...
describe("JsonLogWriter", () => {
  ...
  it("writes event as newline-delimited JSON", async () => {
    ...
    const event: WorkflowEvent = { type: "phase:start", phase: "REQUIREMENTS", timestamp: "2026-01-01T00:00:00.000Z" };
  ...
  it("each line is valid JSON terminated by newline", async () => {
    ...
    await writer.write({ type: "workflow:failed", phase: "DESIGN", error: "test error" });
    await writer.close();
