...
describe("createRunDependencies", () => {
  it("returns a useCase and eventBus", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: false });
    expect(deps.useCase).toBeDefined();
    expect(deps.eventBus).toBeDefined();
  });

  it("returns null logWriter when no logJsonPath provided", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: false });
    expect(deps.logWriter).toBeNull();
  });

  it("returns a logWriter when logJsonPath is provided", async () => {
    const deps = await createRunDependencies(stubConfig, {
      debug: false,
  ...
  it("returns null debugWriter when debug is false", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: false });
    expect(deps.debugWriter).toBeNull();
  });

  it("returns a non-null debugWriter when debug is true", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: true });
    expect(deps.debugWriter).not.toBeNull();
    deps.debugWriter?.close().catch(() => {});
  });

  it("returns a logger in RunDependencies", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: false });
    expect(deps.logger).toBeDefined();
    expect(deps.logger).not.toBeNull();
  });

  it("returns a ConsoleLogger instance as logger", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: false });
    expect(deps.logger).toBeInstanceOf(ConsoleLogger);
  });

  it("uses config.logLevel when debug is false", async () => {
    const configWithWarn: AesConfig = { ...stubConfig, logLevel: "warn" };
    const deps = await createRunDependencies(configWithWarn, { debug: false });
    expect(deps.logger).toBeInstanceOf(ConsoleLogger);
  ...
  it("uses debug level when debug is true", async () => {
    const deps = await createRunDependencies(stubConfig, { debug: true });
    expect(deps.logger).toBeInstanceOf(ConsoleLogger);
