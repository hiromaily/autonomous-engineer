...
export class MockSddAdapter implements SddFrameworkPort {
  ...
  async executeCommand(commandName: string, ctx: SpecContext): Promise<SddOperationResult> {
    switch (commandName) {
      ...
      case "kiro:validate-gap":
        return this.runStub(commandName, ctx, "requirements.md", undefined);
      ...
      case "kiro:validate-design":
        return this.runStub(commandName, ctx, "design.md", undefined);
  ...
  private async runSpecInit(ctx: SpecContext): Promise<SddOperationResult> {
    ...
    try {
      ...
      this.emitOperation("kiro:spec-init", ctx.specName, { ok: true, artifactPath });
      return { ok: true, artifactPath };
    } catch (err) {
      return this.failResult("kiro:spec-init", ctx.specName, err);
    }
  }
