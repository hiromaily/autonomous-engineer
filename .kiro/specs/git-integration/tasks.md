...
- [ ] 2.1 (P) Define the IGitController port and IGitEventBus
  ...
  - Define `IGitController` interface: `listBranches`, `detectChanges`, `createAndCheckoutBranch`, `stageAndCommit`, `push`
  ...
  - Define `IGitEventBus` interface: `emit`, `on`, `off` with `GitEvent` as the typed payload
...
- [ ] 5.5 Implement consecutive-failure escalation and the full-workflow orchestration method
  ...
  - Inject all six interface dependencies and config via constructor: `IGitController`, `IPullRequestProvider`, `LlmProviderPort`, `IGitEventBus`, `IAuditLogger`, `IGitValidator`, `GitIntegrationConfig`
  - _Requirements: 6.5_
