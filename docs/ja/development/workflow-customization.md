...
`custom-sddfw-flow-management` 機能の実装以降、**すべてのフェーズ動作はデータとして定義**されており、複数のソースファイルにハードコードされるのではなく、フレームワーク定義ファイルで管理されます。ワークフローの動作を変更するには、主にSSDフレームワークのフレームワーク定義ファイルを編集します。

> **注意:** フレームワーク定義は現在TypeScriptファイルとして記述されています。TypeScriptの知識なしに編集できるよう、YAMLサポートを将来の機能として計画しています。
...
| 変更したい内容 | 変更場所 |
|---|---|
| フェーズの一覧・順序・タイプ・プロンプト・必要成果物・承認ゲート | フレームワーク定義ファイル（例: `orchestrator-ts/src/infra/sdd/cc-sdd-framework-definition.ts`） |
...
| フレームワークローダー（TypeScriptインポート戦略） | `orchestrator-ts/src/infra/config/typescript-framework-definition-loader.ts` |
...
各SDDフレームワークのフェーズ動作は、`FrameworkDefinition` インターフェースを実装した単一のTypeScriptファイルで定義されます:

**ドメインインターフェース:** `orchestrator-ts/src/domain/workflow/framework.ts`

```typescript
export interface PhaseDefinition {
  readonly phase: WorkflowPhase;         // フェーズ識別子
...
**cc-sdd具体定義:** `orchestrator-ts/src/infra/sdd/cc-sdd-framework-definition.ts`

これはcc-sddワークフローの**唯一の信頼できる情報源**であり、以前 `workflow-engine.ts` と `phase-runner.ts` に分散していた `REQUIRED_ARTIFACTS`、`APPROVAL_GATE_PHASES`、`WORKFLOW_PHASES` 定数を置き換えます。
...
| `human_interaction` | 即座に `{ ok: true }` を返す; 承認ゲート（設定されている場合）が一時停止を処理 |
...
| `HUMAN_INTERACTION` | `human_interaction` | スタブ — 承認ゲートがユーザーの `requirements.md` 作成まで一時停止 |
...
必要成果物はフェーズごとに定義ファイルで宣言されます。例えば `VALIDATE_DESIGN` は実行前に `design.md` の存在が必要です。
...
1. フレームワーク定義ファイルを作成: `orchestrator-ts/src/infra/sdd/open-spec-framework-definition.ts`
2. フレームワークのフェーズで `FrameworkDefinition` インターフェースを実装。
3. `TypeScriptFrameworkDefinitionLoader`（`orchestrator-ts/src/infra/config/typescript-framework-definition-loader.ts`）に登録。
4. `.aes/config.json` に `"sddFramework": "open-spec"` を設定。

不明なフレームワーク識別子が設定されている場合、オーケストレーターは起動時に利用可能なフレームワークを列挙したエラーで失敗します。

---

## cc-sddフェーズ動作の変更

**プロンプトを変更する**場合、`cc-sdd-framework-definition.ts` の `llm_prompt` フェーズの `content` フィールドを編集します。コンテンツはランタイムプレースホルダーとして `{specDir}` をサポートします。

**必要成果物を追加・削除する**場合、該当フェーズの `requiredArtifacts` を更新します。

**承認ゲートを追加・削除する**場合、フェーズ定義の `approvalGate` を設定またはクリアします。有効なゲート値: `"human_interaction"`、`"requirements"`、`"design"`、`"tasks"`。

**フェーズを並び替える**場合、`phases` フィールドの配列順序を変更します。`WorkflowEngine` はこの配列から実行順序を導きます。

定義を編集した後、テストスイート（`bun test`）を実行して構造的な妥当性を確認してください — `validateFrameworkDefinition()` はロード時に呼び出され、フェーズの重複や `llm_slash_command`/`llm_prompt` フェーズの空の `content` などの問題を検出します。
...
SPEC_INIT → llm_slash_command (kiro:spec-init)
  ↓
HUMAN_INTERACTION → human_interaction スタブ → [一時停止: ユーザーが requirements.md を作成]
  ↓ (承認後)
