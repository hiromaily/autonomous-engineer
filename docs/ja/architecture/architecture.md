...
- `InitializeSpecUseCase` — 仕様初期化フローを主導する
- `ExecuteTaskUseCase` — 実装 → レビュー → 改善 → コミットのループを管理する
- `ValidateDesignUseCase` — レビュアーをまたいで設計検証を調整する

ユースケースはコードレベルではドメインインターフェースのみに依存し、アダプターやインフラストラクチャのパッケージを直接インポートすることはありません。実行時には、依存性注入（DI）によって具体的なアダプター実装が注入され、ユースケースはそれらのインターフェースを通じてインフラストラクチャにアクセスします。これにより、特定の実装への結合を避けることができます。
